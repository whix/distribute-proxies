...
class ManageProxies(object):
    def __init__(self):
        ...
        #               'http://api.kxdaili.com/?api=20151111200040011100970070682575&ct=1000']
        self.urls1 = ['http://api.kxdaili.com/?api=20151111200040011100970070682575&ct=1000']
        ...
        self.proxy_url = {}
        self.conn = sqlite3.connect(os.path.join(os.path.dirname(os.path.abspath('__file__')), 'db.sqlite3'))
    ...
    def get_page_proxies(self):
        ...
        for url1 in self.urls1:
            try:
                ...
                html1 = requests.get(url=url1, headers=headers).content
    ...
    def start_working_helper(self):
        while True:
            ...
            print len(self.proxies_fetched)
            self.split_proxies_fetched()
            print len(self.proxies_split)
            for lis in self.proxies_split:
                beg = datetime.now()
                self.filter_proxies(lis)
                print "filter_proxies function cost %s." % (datetime.now() - beg)
                beg = datetime.now()
                self.update_db()
                print "update_db function cost %s." % (datetime.now() - beg)
            print 'ready to execute update_domain_quantity function'
